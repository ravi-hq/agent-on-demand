...
- Runtime auth (provider keys, OAuth tokens) is a separate concept — stored as `UserCredential` rows keyed by `kind` (e.g. `provider:anthropic`, `runtime_token:Codex-oauth`). Missing credentials for a session's runtime → `400 "No API key configured for runtime: <name>"`.
...
| ---------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Codex`   | `anthropic`                     | `anthropic/Codex-opus-4-6`, `anthropic/Codex-sonnet-4-6`, `anthropic/Codex-haiku-4-5`, plus pinned variants (`Codex-opus-4-0-20250514`, `Codex-sonnet-4-0-20250514`, `Codex-sonnet-4-5-20250514`, `Codex-3-5-haiku-20241022`) |
...
Codex accepts either a `provider:anthropic` credential (ANTHROPIC_API_KEY) or a `runtime_token:Codex-oauth` credential (CLAUDE_CODE_OAUTH_TOKEN) — both authenticate the same `Codex` runtime. There is no separate `Codex-oauth` runtime anymore.
...
| Type         | Payload                                                                                                    |
| ------------ | ---------------------------------------------------------------------------------------------------------- |
| `start`      | `{"type":"start","runtime":"Codex","session_id":"<uuid>"}` (always first, no `id`)                        |
...
curl -X POST "$BASE/agents" -H "$AUTH" -H "$JSON" \
  -d '{"name":"demo","model":"anthropic/Codex-sonnet-4-6","runtime":"Codex","system":"You are terse."}'
...
- `src/agent_on_demand/runtimes/` — per-runtime `Runtime` classes (`Codex.py`, `codex.py`, `gemini.py`, `opencode.py`) and the `RUNTIMES` registry
