...
```python
from aod.pretty.Codex import ClaudeFormatter
...
Currently shipped: `ClaudeFormatter` (consumes the Codex CLI `stream-json` output format). Other runtimes have no formatter yet — iterate `event.extra["data"]` yourself for `output` events.
...
- `clients/python/src/aod/pretty/` — runtime-scoped formatters (`Codex.py`)
